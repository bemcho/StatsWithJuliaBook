### This file is copy pasted from https://github.com/alexhallam/Stemplot.jl
### All credits go to https://github.com/alexhallam
### Minor modifications from bemcho to make it work with julia 1.1
using StatsBase
struct Stemplot
  left_ints::Vector{AbstractFloat}
  leaves::Vector{AbstractFloat}
  sample_skew::AbstractFloat

  function Stemplot(v::AbstractVector{T}; scale=10) where {T<:Real}
    v = convert(Vector{AbstractFloat}, v)

    divRemResult =  divrem.(v, scale)
    left_ints = first.(divRemResult)
    leaves = last.(divRemResult)
   
    new(left_ints, leaves, skewness(v))
  end
...
stemplot_getlabel(s) = string(Int(s))
...
function stemplot(plt::Stemplot;
    ...
    println("skewness is $(plt.sample_skew)")
...
# Single
function stemplot(v::AbstractVector{T}; scale=10, args...) where {T<:Real}
  # Stemplot object
...
# Back to back
function stemplot(v1::AbstractVector{T}, v2::AbstractVector; scale=10, args...) where {T<:Real}
  # Stemplot object
